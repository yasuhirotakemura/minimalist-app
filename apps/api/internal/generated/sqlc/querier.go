...
type Querier interface {
	...
	AggregateItemCountsByCategory(ctx context.Context, userID int64) ([]AggregateItemCountsByCategoryRow, error)
	// ダッシュボードの必要度別内訳 (設計書 9.3)。
	// 表示順はDomainのcode体系が決めるため、SQLではcode順に返す。
	AggregateItemCountsByNecessityLevel(ctx context.Context, userID int64) ([]AggregateItemCountsByNecessityLevelRow, error)
	// ダッシュボードの使用頻度別内訳 (設計書 9.3)。
	AggregateItemCountsByUsageFrequency(ctx context.Context, userID int64) ([]AggregateItemCountsByUsageFrequencyRow, error)
	// ダッシュボードの合計 (設計書 9.3)。
	//
	// item_type_count はアイテム種別 (行) の数、total_quantity は所有数量の合計。
	// archive済みは集計へ含めない。
	AggregateItemTotals(ctx context.Context, userID int64) (AggregateItemTotalsRow, error)
